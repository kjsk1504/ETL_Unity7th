            ...
            _confirm.onClick.AddListener(() =>
            {
                FirebaseAuth.DefaultInstance.CreateUserWithEmailAndPasswordAsync(_id.text, _pw.text)
                                            .ContinueWithOnMainThread(task =>
                                            {
                                                if (task.IsCanceled)
                                                {
                                                    Debug.LogError($"[UIRegisterWindew] : Canceled register {_id.text}");
                                                    return;
                                                }
                                                ...
                                                if (task.IsFaulted)
                                                {
                                                    ...
                                                    {
                                                        Debug.LogError($"[UIRegisterWindew] : Faulted register {_id.text}, {task.Exception}");
                                                    }
                                                    return;
                                                }

                                                AuthResult result = task.Result;
                                                UIManager.instance.Get<UIWarningWindow>().Show($"Registered {_id.text}");
                                                Hide();
                                            });
            });
